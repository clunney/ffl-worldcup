...
function BracketViewer({bracket,results,onClose}){
  ...
  const KnockoutTab=()=>{let correct=0,pending=0;return(<div>{LEGEND}{ROUNDS.map(round=>{const picks=ko[round.id]||{},actual=koR[round.id]||{};return(<div key={round.id} style={{marginBottom:12}}><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:13,color:C.accent,letterSpacing:1,marginBottom:6}}>{round.fullLabel}</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:6}}>{Array.from({length:round.n},(_,i)=>{const pick=picks[i],act=actual[i];if(!pick)return null;const isCorrect=act&&pick.code===act.code,isPending=!act;if(isPending)pending++;else if(isCorrect)correct++;return(<div key={i} style={{background:C.card2,borderRadius:7,padding:"7px 10px",display:"flex",alignItems:"center",gap:8,border:"0.5px solid "+C.border}}><Flag code={pick.code} size={20}/><span style={{color:isCorrect?C.accent:act?C.muted:C.text,fontFamily:"'Barlow',sans-serif",fontSize:11,flex:1,fontWeight:isCorrect?600:400}}>{pick.name}</span>{act&&!isCorrect&&<><span style={{color:C.muted,fontSize:9}}>{">"}</span><Flag code={act.code} size={16}/></>}<PtsTag pts={isPending?null:isCorrect?round.pts:0} pending={isPending}/></div>);})}</div></div>);})<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:10}}>{[[correct,C.green,"correct"],[pending,C.muted,"pending/wrong"]].map(([v,c,l])=>(<div key={l} style={{background:C.card2,borderRadius:8,padding:10,textAlign:"center"}}><div style={{color:c,fontSize:20,fontWeight:500}}>{v}</div><div style={{color:C.muted,fontSize:10,marginTop:2}}>{l}</div></div>))}</div></div>);};
