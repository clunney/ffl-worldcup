// App.jsx - World Cup Challenge 2026 - v2
...
const WC_GROUPS = {
  ...
  D:[{name:"USA",code:"us"},{name:"Paraguay",code:"py"},{name:"Australia",code:"au"},{name:"Turkiye",code:"tr"}],
  E:[{name:"Germany",code:"de"},{name:"Curacao",code:"cw"},{name:"Ivory Coast",code:"ci"},{name:"Ecuador",code:"ec"}],
...
const NAME_TO_CODE = {
  "Mexico":"mx","South Africa":"za","Korea Republic":"kr","South Korea":"kr","Czechia":"cz",
  "Canada":"ca","Switzerland":"ch","Qatar":"qa","Bosnia and Herzegovina":"ba","Bosnia & Herz.":"ba",
  "Brazil":"br","Morocco":"ma","Haiti":"ht","Scotland":"gb-sct",
  "United States":"us","USA":"us","Paraguay":"py","Australia":"au","Turkiye":"tr","Turkey":"tr",
...
const toET = u => {
  if(!u) return "";
  return new Date(u).toLocaleString("en-US",{timeZone:"America/New_York",month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:true});
};
const toETTime = u => {
  if(!u) return "";
  return new Date(u).toLocaleString("en-US",{timeZone:"America/New_York",hour:"numeric",minute:"2-digit",hour12:true});
};
...
function computeStats(allBrackets) {
  ...
  const champDist = Object.entries(cc)
...
function computeMatchEdge(apiMatch, allBrackets, results, scoring=DEFAULT_SCORING) {
  ...
  return [{code:homeCode,label:apiMatch.homeTeam?.shortName||apiMatch.homeTeam?.name},
    ...
    const withDelta=currentScores.map(b=>{
      ...
      return {...b,projected:b.score+(picked?roundPts:0)};
    });
    const sorted=[...withDelta].sort((a,b)=>b.projected-a.projected);
    return {winner,standings:sorted.map((b,newPos)=>({
      ...b, newPos:newPos+1,
...
const Flag = ({code,size=28}) => (
  <img
    src={"https://flagcdn.com/w"+(size<=24?"20":"40")+"/"+code+".png"}
...
const Spinner = () => (
  <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:C.bg}}>
    <div style={{textAlign:"center"}}>
      <WCCLogo size={72}/>
      <p style={{color:C.muted,fontFamily:"'Barlow',sans-serif",marginTop:16}}>Loading...</p>
    </div>
  </div>
);
    ...
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      {sent ? (
        <div style={{background:C.card,borderRadius:14,padding:28,maxWidth:340,width:"100%",textAlign:"center",border:"1px solid "+C.borderAccent}}>
          <div style={{fontSize:44,marginBottom:12}}>Email</div>
...
function PoolLanding({userId,pools,poolsLoading,onSelectPool,onCreatePool}) {
  const [joinInput,setJoinInput] = useState(""), [joinPassword,setJoinPassword] = useState("");
  const [joinError,setJoinError] = useState(""), [joining,setJoining] = useState(false);
  ...
  const handleJoin = async () => {
    ...
    if(pool.invite_expires_at&&new Date()>new Date(pool.invite_expires_at)) {
      setJoinError("This pool's invite link has expired."); setJoining(false); return;
    }
    ...
    <div style={{minHeight:"100vh",background:C.bg,padding:20,paddingTop:80}}>
      <div style={{maxWidth:500,margin:"0 auto"}}>
        ...
        <div style={{background:C.card,borderRadius:12,padding:"14px 16px",border:"1px solid "+C.border,marginTop:16}}>
          ...
          <button onClick={handleJoin} disabled={joining||!joinInput.trim()} style={{...btn(true,joining||!joinInput.trim()),width:"100%"}}>
...
function CreatePoolModal({userId,displayName,onCreated,onClose}) {
  const [name,setName] = useState(""), [code,setCode] = useState(""), [password,setPassword] = useState("");
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",zIndex:700,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:C.card,borderRadius:16,padding:24,maxWidth:400,width:"100%",border:"1px solid "+C.borderAccent,maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
          {[["50","max members"],["1","entry per user"]].map(([v,l]) => (
            <div key={l} style={{background:C.card2,borderRadius:8,padding:"8px",textAlign:"center"}}>
              <div style={{color:C.text,fontFamily:"'Bebas Neue',sans-serif",fontSize:20}}>{v}</div>
              <div style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:10}}>{l}</div>
            </div>
          ))}
        </div>
        {error && <p style={{color:C.red,fontFamily:"'Barlow',sans-serif",fontSize:12,marginBottom:10}}>{error}</p>}
        <button onClick={create} disabled={saving||!name||!code} style={{...btn(true,saving||!name||!code),width:"100%"}}>
          {saving?"CREATING...":"CREATE POOL & COPY LINK"}
        </button>
...
function GroupCardMobile({groupId,teams,onReorder,locked,actualGroup}) {
  ...
  const reset = () => {
    if(locked) return;
    onReorder([...WC_GROUPS[groupId]]);
    setMode("pick"); setSwapSel(null);
  };
  ...
  return (
    <div style={{background:C.card,borderRadius:12,padding:12,border:"1px solid "+C.borderAccent,marginBottom:10}}>
      ...
      {mode==="swap"&&!locked && (
        <div style={{color:C.amber,fontFamily:"'Barlow',sans-serif",fontSize:11,marginBottom:8,padding:"6px 8px",background:"rgba(245,158,11,.08)",borderRadius:6}}>Tap two teams to swap their positions</div>
      )}
      {teams.map((team,i) => {
        ...
        return (
          <div key={team.code} onClick={()=>handleTap(team)} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 8px",marginBottom:i<3?5:0,background:isSwapSel?"rgba(6,182,212,.2)":i===0?"rgba(6,182,212,.1)":"rgba(255,255,255,.03)",borderRadius:8,border:"1px solid "+(isSwapSel?C.accent:i===0?C.accentDim:"transparent"),cursor:locked||hasLive?"default":mode==="swap"?"pointer":"default"}}>
            ...
            {hasLive && (
              <div style={{display:"flex",alignItems:"center",gap:5,flexShrink:0}}>
                <span style={{fontSize:13,color:correct?C.green:C.red}}>{correct?"OK":"X"}</span>
                {actualTeam&&actualTeam.code!==team.code && <Flag code={actualTeam.code} size={16}/>}
              </div>
            )}
            {!locked&&!hasLive && (
              <div style={{display:"flex",flexDirection:"column",gap:2}}>
                {[[-1,"up"],[1,"dn"]].map(([dir,sym]) => (
                  <button key={sym} onClick={e=>{e.stopPropagation();move(i,dir);}} disabled={(dir===-1&&i===0)||(dir===1&&i===3)} style={{background:"transparent",border:"1px solid "+C.accentDim,borderRadius:3,width:22,height:17,color:((dir===-1&&i===0)||(dir===1&&i===3))?C.muted:C.accent,cursor:"pointer",fontSize:9,display:"flex",alignItems:"center",justifyContent:"center"}}>
                    {dir===-1?"^":"v"}
                  </button>
                ))}
              </div>
  ...
  return (
    <div style={{background:C.card,borderRadius:12,padding:12,border:"1px solid "+C.borderAccent}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
        ...
        <span style={{color:C.accent,fontFamily:"'Bebas Neue',sans-serif",fontSize:13,letterSpacing:1}}>GROUP {groupId}</span>
        {hasLive && <span style={{marginLeft:"auto",color:C.green,fontFamily:"'Barlow',sans-serif",fontSize:10,fontWeight:600}}>FINAL</span>}
        {!locked&&!hasLive && <span style={{marginLeft:"auto",color:C.muted,fontSize:10,fontFamily:"'Barlow',sans-serif"}}>drag or use arrows</span>}
      </div>
      {teams.map((team,i) => {
        ...
        return (
          <div key={team.code}
            ...
            {hasLive && (
              <div style={{display:"flex",alignItems:"center",gap:5,flexShrink:0}}>
                <span style={{fontSize:12,color:correct?C.green:C.red}}>{correct?"OK":"X"}</span>
                {actualTeam&&actualTeam.code!==team.code && <Flag code={actualTeam.code} size={16}/>}
              </div>
            )}
            {!locked&&!hasLive && (
              <div style={{display:"flex",flexDirection:"column",gap:2}}>
                {[[-1,"^"],[1,"v"]].map(([dir,sym]) => (
                  <button key={sym} onClick={e=>{e.stopPropagation();move(i,dir);}} disabled={(dir===-1&&i===0)||(dir===1&&i===3)} style={{background:"transparent",border:"1px solid "+C.accentDim,borderRadius:3,width:20,height:16,color:((dir===-1&&i===0)||(dir===1&&i===3))?C.muted:C.accent,cursor:"pointer",fontSize:8,display:"flex",alignItems:"center",justifyContent:"center"}}>
                    {sym}
                  </button>
                ))}
              </div>
    ...
    <div style={{paddingBottom:90}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(250px,1fr))",gap:10,padding:12}}>
        {thirds.map(({group,team}) => {
          ...
          return (
            <button key={team.code} onClick={()=>toggle(team.code)} disabled={disabled||locked} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",background:sel?"rgba(6,182,212,0.13)":C.card,border:"1.5px solid "+(sel?C.accent:C.border),borderRadius:10,cursor:disabled||locked?"not-allowed":"pointer",opacity:disabled?0.38:1,textAlign:"left",transition:"all .15s"}}>
              {!hasActual && (
                <div style={{width:20,height:20,borderRadius:"50%",border:"2px solid "+(sel?C.accent:C.muted),background:sel?C.accent:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  {sel && <span style={{color:"#0a0e1a",fontSize:10,fontWeight:700}}>OK</span>}
                </div>
              )}
...
function MatchPickCard({num,team1,team2,winner,onPick,locked,actualWinner}) {
  const Team = ({team}) => {
    if(!team) return (
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"9px 12px",opacity:.3}}>
        <div style={{width:26,height:17,background:C.card2,borderRadius:2}}/>
        <span style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:13}}>TBD</span>
      </div>
    );
    ...
    return (
      <button onClick={()=>!locked&&onPick(team)} style={{display:"flex",alignItems:"center",gap:10,width:"100%",padding:"9px 12px",background:isW?"rgba(6,182,212,0.15)":"transparent",border:isW?"1px solid "+C.accentDim:"1px solid transparent",borderRadius:8,cursor:locked?"default":"pointer",opacity:isL?.35:1,transition:"all .15s",textAlign:"left"}}>
        ...
        {isW && <span style={{color:C.accent,fontSize:13}}>OK</span>}
      </button>
    );
  };
  return (
...
function KnockoutPage({groupPicks,wildcardPicks,knockoutPicks,setKnockoutPicks,locked,onBack,results,championGoalDiff,setChampionGoalDiff,triggerSave}) {
  ...
  const sfLosers = [0,1].map(i=>{
    try {
      const [t1,t2]=getTeams("sf",i), w=knockoutPicks.sf?.[i];
    ...
    <div style={{paddingBottom:90}}>
      <div style={{padding:"14px 14px 0",background:C.bg,position:"sticky",top:58,zIndex:9,borderBottom:"1px solid "+C.borderAccent}}>
        ...
        <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:12}}>
          {[...ROUNDS,{id:"final",label:"Final",n:1}].map(r => {
            ...
            return (
              <button key={r.id} onClick={()=>unlocked&&changeRound(r.id)} style={{flexShrink:0,padding:"6px 14px",background:activeRound===r.id?C.accent:unlocked?C.card:C.card2,color:activeRound===r.id?"#0a0e1a":unlocked?C.text:C.muted,fontFamily:"'Bebas Neue',sans-serif",fontSize:12,letterSpacing:.5,border:"1px solid "+(complete?C.green+"55":activeRound===r.id?C.accent:C.border),borderRadius:20,cursor:unlocked?"pointer":"not-allowed",whiteSpace:"nowrap"}}>
        ...
        {activeRound!=="final" && (
          <p style={{color:C.muted,fontSize:12,fontFamily:"'Barlow',sans-serif",paddingBottom:10}}>{picked}/{currentRound.n} picked - tap a team to advance them</p>
        )}
      </div>
      ...
      {activeRound==="final" && (
        <div style={{padding:12,display:"flex",flexDirection:"column",gap:12}}>
          <div>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
              <span style={{fontSize:22}}>Trophy</span>
          ...
          <div>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
              <span style={{fontSize:20}}>Bronze</span>
          ...
          {knockoutPicks.champion && (
            <Card accent style={{textAlign:"center",padding:20}}>
              ...
              {koR.champion && (
                <div style={{fontSize:13,fontFamily:"'Barlow',sans-serif",color:koR.champion.code===knockoutPicks.champion.code?C.green:C.red,marginBottom:12}}>
              ...
              <div style={{background:C.card2,borderRadius:10,padding:14,textAlign:"left"}}>
                ...
                <div style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:12,marginBottom:10,lineHeight:1.5}}>
                  Predict <strong style={{color:C.text}}>{knockoutPicks.champion.name}'s</strong> total goal differential across all their matches (goals scored minus goals conceded). Used to break ties in the final standings.
                </div>
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  ...
                  {results?.actual_champion_goal_diff!=null && (
                    <div style={{textAlign:"right"}}>
                      <div style={{color:C.muted,fontSize:10,fontFamily:"'Barlow',sans-serif"}}>Actual</div>
                ...
                {championGoalDiff==null&&!locked && (
                  <p style={{color:C.amber,fontFamily:"'Barlow',sans-serif",fontSize:11,marginTop:8}}>Enter your tiebreaker guess to complete your bracket</p>
                )}
              </div>
            </Card>
          )}
          ...
          return (
            <div key={g} style={{background:C.card2,borderRadius:8,padding:10,border:"1px solid "+C.borderAccent}}>
              ...
              {picks.map((team,i) => {
                ...
                return (
                  <div key={team.code} style={{display:"flex",alignItems:"center",gap:6,padding:"4px 0",borderBottom:i<3?"0.5px solid "+C.border:"none"}}>
      ...
      <div>
        {champ ? (
          <Card accent style={{textAlign:"center",padding:20,marginBottom:12}}>
            ...
            {actual
              ? <div style={{fontSize:14,color:correct?C.green:C.red,fontFamily:"'Barlow',sans-serif"}}>{correct?"Champion! +20 pts":actual.name+" won"}</div>
              : <div style={{color:C.muted,fontSize:12}}>Tournament in progress</div>
            }
            {bracket.champion_goal_diff_pick!=null && (
              <div style={{marginTop:10,padding:8,background:C.card2,borderRadius:8}}>
                <div style={{color:C.muted,fontSize:11,fontFamily:"'Barlow',sans-serif"}}>
                  Tiebreaker guess: <strong style={{color:C.accent}}>{bracket.champion_goal_diff_pick>0?"+":""}{bracket.champion_goal_diff_pick}</strong>
                </div>
                {results?.actual_champion_goal_diff!=null && (
                  <div style={{color:C.muted,fontSize:11,marginTop:4}}>
                    {Math.abs(bracket.champion_goal_diff_pick-results.actual_champion_goal_diff)} off actual ({results.actual_champion_goal_diff>0?"+":""}{results.actual_champion_goal_diff})
                  </div>
                )}
              </div>
            )}
        ...
        <Card style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,textAlign:"center"}}>
          <div>
            <div style={{color:C.accent,fontFamily:"'Bebas Neue',sans-serif",fontSize:26}}>{score.total}</div>
            <div style={{color:C.muted,fontSize:11}}>current pts</div>
          </div>
          <div>
            <div style={{color:C.muted,fontFamily:"'Bebas Neue',sans-serif",fontSize:26}}>---</div>
            <div style={{color:C.muted,fontSize:11}}>projected</div>
          </div>
        </Card>
      </div>
    );
              ...
              return (
                <div key={g} style={{background:C.card2,borderRadius:8,padding:10,border:"0.5px solid "+C.border}}>
                  ...
                  {[0,1,2,3].map(i => {
                    ...
                    return (
                      <div key={i} style={{display:"flex",alignItems:"center",gap:6,padding:"4px 0",borderBottom:i<3?"0.5px solid "+C.border:"none"}}>
          ...
          <div>
                      ...
                      return (
                        <div key={idx} style={{background:C.card2,borderRadius:7,padding:"8px 10px",border:"0.5px solid "+(same?C.border:C.amber+"44")}}>
                          <div style={{display:"flex",alignItems:"center",gap:6}}>
                            ...
                            {!same&&opPick && <><span style={{color:C.amber,fontSize:10}}>{"!="}</span><span style={{color:act?.code===opPick.code?C.green:act?C.red:C.text,fontSize:11,fontFamily:"'Barlow',sans-serif",flex:1,textAlign:"right"}}>{opPick.name}</span><Flag code={opPick.code} size={18}/></>}
            ...
            <div style={{display:"flex",gap:10,marginTop:8}}>
              {[myBracket,opponent].map((b,idx) => {
                const champ=b.knockout_picks?.champion, actual=koR.champion;
                ...
                return champ ? (
                  <div key={idx} style={{flex:1,background:C.card2,borderRadius:8,padding:"10px 12px",textAlign:"center",border:"1px solid "+(correct?C.green+"44":C.borderAccent)}}>
                    <div style={{color:C.muted,fontSize:10,marginBottom:6}}>Trophy {idx===0?"YOUR":"THEIR"} CHAMPION</div>
      ...
      {liveMatch ? (
        <div style={{background:"rgba(239,68,68,.08)",border:"1px solid rgba(239,68,68,.25)",borderRadius:12,padding:"12px 16px",marginBottom:8}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8,justifyContent:"center"}}>
            ...
            <span style={{color:C.red,fontFamily:"'Bebas Neue',sans-serif",fontSize:13,letterSpacing:1}}>LIVE - {liveMatch.status==="HALFTIME"?"HT":liveMatch.minute+"min"}</span>
          </div>
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,flexWrap:"wrap"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span style={{color:C.text,fontFamily:"'Bebas Neue',sans-serif",fontSize:24}}>{liveMatch.score?.fullTime?.home??0}</span>
              <span style={{color:C.muted}}>:</span>
              <span style={{color:C.text,fontFamily:"'Bebas Neue',sans-serif",fontSize:24}}>{liveMatch.score?.fullTime?.away??0}</span>
            </div>
...
function LeaderboardPage({userId,displayName,bracketComplete,bracketName,allBrackets,results,locked,picksVisible,onViewBracket,onH2H,oddsMap,currentPool,matches,globalPercentile}) {
  ...
  const shareMyBracket = () => {
    navigator.clipboard.writeText(window.location.origin+"?viewbracket="+userId).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2000);});
  };
    ...
    <div style={{padding:12,paddingBottom:90}}>
      ...
      <Card accent style={{marginBottom:12}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:globalPercentile!=null?"1fr 1fr 1fr 1fr":"1fr 1fr 1fr",gap:8,marginBottom:10}}>
          <div style={{background:C.card2,borderRadius:8,padding:"10px 6px",textAlign:"center"}}>
            <div style={{color:C.amber,fontFamily:"'Bebas Neue',sans-serif",fontSize:22,lineHeight:1}}>{tournamentStarted?ordinal(myPos):"---"}</div>
            <div style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:10,marginTop:3}}>in pool</div>
          </div>
          <div style={{background:C.card2,borderRadius:8,padding:"10px 6px",textAlign:"center"}}>
            ...
            <div style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:10,marginTop:3}}>pts</div>
          </div>
          <div style={{background:C.card2,borderRadius:8,padding:"10px 6px",textAlign:"center"}}>
            ...
            <div style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:10,marginTop:3}}>proj</div>
          </div>
          {globalPercentile!=null && (
            <div style={{background:C.card2,borderRadius:8,padding:"10px 6px",textAlign:"center"}}>
              <div style={{color:C.accent,fontFamily:"'Bebas Neue',sans-serif",fontSize:22,lineHeight:1}}>{globalPercentile}%</div>
              <div style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:10,marginTop:3}}>global</div>
            </div>
          )}
        </div>
        {globalPercentile!=null&&tournamentStarted && (
          <div style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:11,textAlign:"center",marginBottom:8}}>
            Top {globalPercentile}th percentile across all WCC pools
          </div>
        )}
        <div style={{padding:"8px 12px",background:C.card2,borderRadius:8,display:"flex",alignItems:"center",gap:8}}>
          ...
          <span style={{color:bracketComplete?C.text:C.amber,fontFamily:"'Barlow',sans-serif",fontSize:12,flex:1}}>
            {bracketComplete
              ? <>{bracketName} - <span style={{color:C.green}}>complete</span></>
              : "Your bracket is incomplete - tap Picks to finish"
            }
          </span>
        </div>
      </Card>
            ...
            <div key={b.id} style={{padding:"10px",borderBottom:i<scored.length-1?"1px solid "+C.border:"none",background:isMe?"rgba(6,182,212,.06)":"transparent",borderRadius:6}}>
              <div style={{display:"grid",gridTemplateColumns:"26px 1fr 46px 54px",gap:6,alignItems:"center"}}>
                ...
                <div onClick={()=>canView&&onViewBracket(b)} style={{cursor:canView?"pointer":"default"}}>
                  ...
                  <div style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:11,display:"flex",alignItems:"center",gap:6}}>
      ...
      <Card>
        ...
        <div style={{marginTop:10,padding:8,background:"rgba(6,182,212,.07)",borderRadius:8,textAlign:"center"}}>
          <span style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:11}}>PROJ = odds favorites winning remaining matches - max ~{MAX_POSSIBLE} pts</span>
        </div>
      </Card>
    </div>
  );
...
function MatchesPage({matches,loading}) {
  ...
  useEffect(()=>{
    fetch("/api/odds").then(r=>r.ok?r.json():null).then(d=>{if(d&&Array.isArray(d))setOdds(d);}).catch(()=>{});
  },[]);

  const getOdds = homeTeam => {
    const m=odds.find(o=>o.home_team?.toLowerCase().includes((homeTeam||"").toLowerCase().split(" ")[0]));
    if(!m) return null;
    const book=m.bookmakers?.[0]?.markets?.[0]?.outcomes;
    if(!book) return null;
  ...
  const MatchRow = ({m,showScore}) => {
    ...
    const status=isLive?(m.status==="HALFTIME"?"HT":m.minute+"min"):"FT";
    const o=!showScore?getOdds(m.homeTeam?.name):null;
    return (
    ...
    <div style={{padding:12,paddingBottom:90}}>
      ...
      {matches.length===0 && (
        <div style={{textAlign:"center",padding:40}}>
          <div style={{fontSize:40,marginBottom:12}}>Calendar</div>
...
function InsightsPage({allBrackets,userId,results,picksVisible,matches}) {
  ...
  if(!picksVisible) return (
    <div style={{padding:24,textAlign:"center",paddingBottom:90}}>
      <div style={{fontSize:44,marginBottom:16}}>Lock</div>
    ...
    <div style={{padding:12,paddingBottom:90}}>
          ...
          {todayKnockout.length===0 && (
            <Card>
              <div style={{textAlign:"center",padding:20}}>
                <div style={{fontSize:36,marginBottom:10}}>Ball</div>
                <p style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:13}}>No knockout matches today. Check back during the knockout rounds.</p>
              </div>
            </Card>
          )}
          {todayKnockout.map(m => {
          ...
          <div style={{marginBottom:12}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
              {ALL_TEAMS.slice(0,16).map(team => (
                <button key={team.code} onClick={()=>setWhatIfTeam(whatIfTeam?.code===team.code?null:team)} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 10px",background:whatIfTeam?.code===team.code?"rgba(6,182,212,.2)":C.card2,border:"1px solid "+(whatIfTeam?.code===team.code?C.accent:C.border),borderRadius:8,cursor:"pointer"}}>
                  <Flag code={team.code} size={18}/>
      ...
      {tab==="pool" && (
        <div>
          ...
          {contrarian.length>0 && (
            <Card>
              <SecHead label="CONTRARIAN PICKS" sub="Champion picks by 25% or fewer of the pool"/>
                ...
                {contrarian.map(({team,pct,count}) => (
                  <div key={team.code} style={{background:C.card2,borderRadius:8,padding:10,border:"1px solid "+C.accentDim}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                      <Flag code={team.code} size={20}/>
                      <span style={{color:C.text,fontFamily:"'Barlow',sans-serif",fontSize:12,fontWeight:600}}>{team.name}</span>
                    </div>
      ...
      {tab==="root" && (
        <Card accent>
          <SecHead label="WHO TO ROOT FOR" sub="Teams in your bracket that most others don't have - if they win, you gain the most ground"/>
...
function PoolManagerPanel({pool,allBrackets,results,onClose,userId}) {
  ...
  const saveScoring = async () => {
    setSaving(true);
    const {error} = await supabase.from("pools").update({scoring_config:sc}).eq("id",pool.id);
    setSaving(false);
    setMsg(error?"Error saving":"Scoring saved - retroactively applied!");
    setTimeout(()=>setMsg(""),3000);
  };

  const removeMember = async (uid) => {
    if(uid===userId){setMsg("You can't remove yourself.");setTimeout(()=>setMsg(""),2000);return;}
    await supabase.from("pool_members").delete().eq("pool_id",pool.id).eq("user_id",uid);
    await supabase.from("brackets").delete().eq("pool_id",pool.id).eq("user_id",uid);
    setMsg("Member removed."); setTimeout(()=>setMsg(""),2000);
  };
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.9)",zIndex:600,overflowY:"auto"}}>
      <div style={{maxWidth:600,margin:"0 auto",padding:12,paddingBottom:80}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
          <div>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,color:C.accent,letterSpacing:1.5}}>POOL MANAGER</div>
            <div style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:12}}>{pool.name}</div>
          </div>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            {msg && <span style={{color:msg.includes("Error")?C.red:C.green,fontFamily:"'Barlow',sans-serif",fontSize:12}}>{msg}</span>}
            <button onClick={onClose} style={{background:"transparent",border:"1px solid "+C.border,borderRadius:8,color:C.muted,padding:"8px 14px",cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:13}}>CLOSE</button>
          </div>
        </div>
        ...
        <Card accent>
          ...
          <div style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:11}}>Expires: Jun 11, 2026 at kickoff</div>
          <button onClick={()=>navigator.clipboard.writeText(window.location.origin+"?join="+pool.code).then(()=>setMsg("Link copied!"))}
            style={{...btn(true),width:"100%",marginTop:10}}>COPY INVITE LINK</button>
        </Card>

        <Card>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:14,color:C.accent,letterSpacing:1,marginBottom:8}}>MEMBERS</div>
          {allBrackets.map(b => (
            <div key={b.user_id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:"1px solid "+C.border}}>
              <div style={{flex:1}}>
                <div style={{color:C.text,fontFamily:"'Barlow',sans-serif",fontSize:13,fontWeight:600}}>{b.display_name||"---"}</div>
                <div style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:11}}>{b.bracket_name}</div>
              </div>
              {b.user_id!==userId && (
                <button onClick={()=>removeMember(b.user_id)} style={{background:"transparent",border:"1px solid rgba(239,68,68,.3)",borderRadius:6,color:C.red,fontFamily:"'Bebas Neue',sans-serif",fontSize:10,padding:"3px 8px",cursor:"pointer"}}>REMOVE</button>
              )}
            </div>
          ))}
        </Card>
        ...
        <Card>
          ...
          <div style={{color:C.amber,fontFamily:"'Barlow',sans-serif",fontSize:11,marginBottom:10,padding:"6px 8px",background:"rgba(245,158,11,.08)",borderRadius:6}}>
            Pools with custom scoring are excluded from the global percentile ranking
          </div>
          ...
          {pool.scoring_config && (
            <button onClick={async()=>{setSaving(true);await supabase.from("pools").update({scoring_config:null}).eq("id",pool.id);setSaving(false);setMsg("Reset to global defaults.");setTimeout(()=>setMsg(""),2500);}}
              disabled={saving} style={{...btn(false,saving),width:"100%",marginTop:8,fontSize:13}}>RESET TO GLOBAL DEFAULTS</button>
          )}
        </Card>
      </div>
    </div>
...
function ManagerPage({allBrackets,results,onResultsUpdate,pools,onPoolCreated,userId}) {
  ...
  const [saving,setSaving] = useState(false), [msg,setMsg] = useState("");
  const [selUser,setSelUser] = useState(""), [editB,setEditB] = useState(null);
  ...
  const Row = ({label,sub,children}) => (
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 0",borderBottom:"1px solid "+C.border,gap:12}}>
      <div>
        <div style={{color:C.text,fontFamily:"'Barlow',sans-serif",fontSize:13,fontWeight:600}}>{label}</div>
        {sub && <div style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:11,marginTop:2}}>{sub}</div>}
      </div>
      {children}
    </div>
  );
  ...
  return (
    <div style={{padding:12,paddingBottom:90}}>
      ...
      <Card accent>
        ...
        <Row label="Picks locked" sub="No edits once enabled">
          <Toggle val={results?.tournament_locked||false} onChange={v=>save("lock",v)}/>
        </Row>
        <Row label="Show all picks publicly" sub="Auto-enables at lock">
          <Toggle val={results?.picks_visible||false} onChange={v=>save("visible",v)}/>
        </Row>
        <Row label="Actual champion goal differential" sub="Enter after tournament ends">
      ...
      <Card>
        ...
        {pools.map(p => (
          <div key={p.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:"1px solid "+C.border}}>
            ...
            <button onClick={()=>{navigator.clipboard.writeText(window.location.origin+"?join="+p.code).then(()=>{setCopied(p.code);setTimeout(()=>setCopied(null),2000);});}}
              style={{background:"transparent",border:"1px solid "+C.accentDim,borderRadius:8,color:copied===p.code?C.green:C.accent,fontFamily:"'Bebas Neue',sans-serif",fontSize:11,padding:"5px 10px",cursor:"pointer",flexShrink:0}}>
      ...
      <Card>
        ...
        <div style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:11,marginBottom:10}}>Applies to all pools that have not set custom scoring</div>
        <PtsIn label="Exact group rank" field="exactPos"/>
        <PtsIn label="Advanced (wrong rank)" field="advancedWrong"/>
        <PtsIn label="Perfect group bonus" field="perfectGroup"/>
        <PtsIn label="Wildcard correct" field="wildcardCorrect"/>
      ...
      <Card>
        ...
        {editB && (
          <div>
            ...
            <button onClick={async()=>{setSaving(true);await supabase.from("brackets").update({bracket_name:editB.bracket_name}).eq("user_id",editB.user_id);setSaving(false);setMsg("Saved!");setTimeout(()=>setMsg(""),2500);}}
              disabled={saving} style={{...btn(true,saving),width:"100%"}}>{saving?"SAVING...":"SAVE"}</button>
          </div>
        )}
      </Card>
    </div>
...
export default function App() {
  ...
  const [joinCode,setJoinCode] = useState(null), [joinPool,setJoinPool] = useState(null);
  const [joinStatus,setJoinStatus] = useState("idle"), [viewSharedUserId,setViewSharedUserId] = useState(null);
  const [bracketId,setBracketId] = useState(null), [bracketName,setBracketName] = useState("My WCC Bracket");
  ...
  useEffect(()=>{
    ...
    const code=params.get("join"), view=params.get("viewbracket");
  ...
  const loadPools = useCallback(async()=>{
    ...
    if(data?.length){
      const ps=data.map(d=>({...d.pools,my_role:d.role})).filter(Boolean);
      setPools(ps);
      if(ps.length===1&&!activePool) setActivePool(ps[0].id);
    } else setPools([]);
  ...
  const fetchMatches = useCallback(async()=>{
    try {
      ...
      if(r.ok){
        ...
        const newData=JSON.stringify(d.matches||[]);
        if(newData!==prevMatchRef.current){prevMatchRef.current=newData;setMatches(d.matches||[]);}
      }
    } catch(e){}
  ...
  useEffect(()=>{
    fetch("/api/odds").then(r=>r.ok?r.json():null).then(data=>{
      ...
      data.forEach(match=>{
        ...
        const fav=sorted[0], code=NAME_TO_CODE[fav?.name];
  ...
  useEffect(()=>{
    ...
    const calcPercentile = async () => {
      ...
      setGlobalPercentile(Math.round((below/allScores.length)*100));
    };
    calcPercentile();
  ...
  if(!activePool) return (
    <>
      ...
      <div style={{position:"fixed",top:12,right:16,zIndex:200}}>
        <button onClick={signOut} style={{background:"transparent",border:"1px solid "+C.border,borderRadius:8,color:C.muted,fontFamily:"'Bebas Neue',sans-serif",fontSize:11,letterSpacing:.5,padding:"5px 10px",cursor:"pointer"}}>SIGN OUT</button>
      </div>
          ...
          <div style={{flex:1,overflow:"hidden",minWidth:0}}>
            <button onClick={()=>setShowPoolSwitcher(v=>!v)} style={{background:"transparent",border:"none",padding:0,cursor:"pointer",textAlign:"left",width:"100%"}}>
              <div style={{display:"flex",alignItems:"center",gap:4}}>
                ...
                <span style={{color:C.accent,fontSize:12,flexShrink:0}}>v</span>
              </div>
            ...
            {page==="bracket"&&saveStatus==="idle" && (editingName
              ...
              : <button onClick={()=>!locked&&setEditingName(true)} style={{background:"transparent",border:"none",color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:11,cursor:locked?"default":"pointer",padding:0}}>{bracketName}{!locked?" (edit)":""}</button>
            )}
          </div>
          <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
            {isPoolManager && (
              <button onClick={()=>setShowPoolManager(true)} style={{background:"transparent",border:"1px solid "+C.accentDim,borderRadius:8,color:C.accent,fontFamily:"'Bebas Neue',sans-serif",fontSize:10,padding:"4px 8px",cursor:"pointer",letterSpacing:.5}}>MANAGE</button>
            )}
          ...
          {navItems.map(({id,label,Icon}) => (
            <button key={id} onClick={()=>{setPage(id);setShowPoolSwitcher(false);}} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"4px 12px",background:"transparent",border:"none",cursor:"pointer",color:page===id?C.accent:C.muted,minWidth:0,position:"relative"}}>
              ...
              {id==="matches"&&(hasLiveMatch||hasMatchesToday)&&page!=="matches" && (
                <div style={{position:"absolute",top:2,right:8,width:7,height:7,borderRadius:"50%",background:hasLiveMatch?C.red:C.accent}}/>
              )}
