...
function deriveResultsFromMatches(matches){
  ...
  finished.filter(m=>m.stage&&stageMap[m.stage]).forEach(m=>{
    ...
    if(round==="thirdPlace"){
      ...
      if(decidedTeams[round]){
        decidedTeams[round].push(wObj.code);
        if(lObj?.code) decidedTeams[round].push(lObj.code);
      }
    }
  });

  // -- Valid eligibility pools per round (which teams could legitimately appear here) --
  // r32 pool = real group top-2 finishers (once that group is finalized) + real wildcards
  const r32Pool=[];
  Object.values(groupResults).forEach(ranked=>{
    if(ranked[0]?.code) r32Pool.push(ranked[0].code);
    if(ranked[1]?.code) r32Pool.push(ranked[1].code);
  });
  wildcardCodes.forEach(c=>r32Pool.push(c));
  // Whether we have enough info to know the full real r32 pool yet (all 12 groups finalized)
  const groupsFinalized=Object.keys(groupResults).length>=12;
  ...
  return {
    ...
    r32_pool:r32Pool,
    groups_finalized:groupsFinalized,
    _derived:true,
  };
}
...
function mergeResults(stored,derived){
  ...
  return {
    ...
    decided_teams:derived.decided_teams||{r32:[],r16:[],qf:[],sf:[]},
    r32_pool:derived.r32_pool||[],
    groups_finalized:!!derived.groups_finalized,
  };
}

// ---- Shared pick-status helpers (used by Picks page, BracketViewer, H2H) ----
// Mirrors calculateScore's exact branching so every display agrees with real standings.
...
function wildcardPickDetail(code,wc,groupsKnown,scoring=DEFAULT_SCORING){
  if(!groupsKnown) return{pts:null,status:"pending"};
  const correct=wc.includes(code);
  return{pts:correct?scoring.wildcardCorrect:0,status:correct?"correct":"wrong"};
}
// Team-based knockout status: correct (won), eliminated (confirmed lost), pending (not yet played)
function knockoutPickStatus(code,round,results){
...
function WildcardPage({groupPicks,wildcardPicks,setWildcardPicks,wildcardRanking,setWildcardRanking,locked,onNext,onBack,results}){
  // Fix stale picks when user goes back and changes group stage
    ...
    <div style={{paddingBottom:90}}>
      ...
      {activeRound==="final"&&(
        <div style={{padding:"0 12px 12px",display:"flex",flexDirection:"column",gap:12}}>
          ...
          {/* Submit button + completion summary */}
          {knockoutPicks.champion&&championGoalDiff!=null&&!locked&&(
            <button onClick={()=>{if(triggerSave)triggerSave();if(onSubmitBracket)onSubmitBracket();}}
              style={{...btn(true),width:"100%",fontSize:17,padding:"16px",background:"linear-gradient(135deg,#f59e0b,#d97706)",color:"#0a0e1a",boxShadow:"0 0 20px rgba(245,158,11,.4)",marginBottom:4}}>
              SUBMIT BRACKET
            </button>
          )}
        ...
        {viewTab==="groups"&&(
          <div style={{padding:12}}>
            <div style={{color:"#475569",fontFamily:"'Barlow',sans-serif",fontSize:10,marginBottom:8,padding:"4px 8px",background:"rgba(255,255,255,.03)",borderRadius:6}}>
              Numbers in grey (e.g. #12) are FIFA world rankings at time of tournament. Green = points earned, red = no points, grey = not yet decided.
            </div>
            {(()=>{
              const gResults=results?.group_results||{},wcCodes=results?.wildcard_codes||[];
              let poolTotal=0;
              const groupBlocks=Object.keys(WC_GROUPS).map(g=>{
                const picks=groupPicks[g]||WC_GROUPS[g];
                const actual=gResults[g]||[];
                const details=picks.map((team,i)=>groupPickDetail(team,i,actual,wcCodes));
                const exactCount=details.filter(d=>d.status==="exact").length;
                const perfect=actual.length>0&&exactCount===4;
                const gSubtotal=details.reduce((acc,d)=>acc+(d.pts||0),0)+(perfect?DEFAULT_SCORING.perfectGroup:0);
                poolTotal+=gSubtotal;
                return{g,picks,actual,details,perfect,gSubtotal};
              });
              return(
                <>
                  {groupBlocks.map(({g,picks,actual,details,perfect,gSubtotal})=>(
                    <div key={g} style={{background:C.card,borderRadius:10,padding:12,marginBottom:8,border:"1px solid "+C.border}}>
                      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:12,color:C.accent,letterSpacing:1}}>GROUP {g}</div>
                        {actual.length>0&&<span style={{color:C.accent,fontFamily:"'Bebas Neue',sans-serif",fontSize:13}}>+{gSubtotal} pts</span>}
                      </div>
                      {picks.map((team,i)=>{
                        const d=details[i];
                        const actualTeam=actual[d.actualIdx];
                        const color=d.status==="pending"?C.text:d.status==="none"?C.red:C.green;
                        return(
                          <div key={team.code} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 0",borderBottom:i<3?"0.5px solid "+C.border:"none"}}>
                            ...
                            <span style={{color,fontFamily:"'Barlow',sans-serif",fontSize:12,flex:1}}>{team.name}</span>
                            <FifaRank code={team.code}/>
                            {d.status!=="pending"&&d.status!=="exact"&&actualTeam&&actualTeam.code!==team.code&&(
                              <span style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:9}}>{"(actual: "+["1st","2nd","3rd","4th"][d.actualIdx]+")"}</span>
                            )}
                            <PtsTag pts={d.pts} pending={d.status==="pending"}/>
                          </div>
                        );
                      })}
                      {perfect&&<div style={{color:C.green,fontFamily:"'Barlow',sans-serif",fontSize:10,marginTop:5,textAlign:"center"}}>{"+"+DEFAULT_SCORING.perfectGroup+" perfect bonus!"}</div>}
                    </div>
                  ))}
                  <div style={{background:"rgba(6,182,212,.08)",border:"1px solid "+C.borderAccent,borderRadius:10,padding:"10px 14px",textAlign:"center",marginTop:4}}>
                    <span style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:12}}>Group stage subtotal: </span>
                    <span style={{color:C.accent,fontFamily:"'Bebas Neue',sans-serif",fontSize:16}}>{poolTotal} pts</span>
                  </div>
                </>
              );
            })()}
        ...
        {viewTab==="wildcards"&&(
          <div style={{padding:12}}>
            ...
            {(()=>{
              ...
              const groupsKnown=Object.keys(results?.group_results||{}).length>=12;
              const wcList=wildcardRanking.length>0?wildcardRanking:wildcardPicks.map(code=>({code,name:Object.values(WC_GROUPS).flat().find(t=>t.code===code)?.name||code}));
              let wcSubtotal=0;
              const rows=wcList.map((team,i)=>{
                const d=wildcardPickDetail(team.code,wcCodes,groupsKnown);
                wcSubtotal+=d.pts||0;
                return{team,i,d};
              });
              return(
                <>
                  {rows.map(({team,i,d})=>{
                    const color=d.status==="pending"?C.text:d.status==="wrong"?C.red:C.green;
                    return(
                      <div key={team.code} style={{display:"flex",alignItems:"center",gap:10,padding:"10px",background:C.card,borderRadius:8,marginBottom:6,border:"1px solid "+C.border}}>
                        ...
                        <Flag code={team.code} size={22}/>
                        <span style={{color,fontFamily:"'Barlow',sans-serif",fontSize:13,flex:1}}>{team.name}</span>
                        <FifaRank code={team.code}/>
                        <PtsTag pts={d.pts} pending={d.status==="pending"}/>
                      </div>
                    );
                  })}
                  <div style={{background:"rgba(6,182,212,.08)",border:"1px solid "+C.borderAccent,borderRadius:10,padding:"10px 14px",textAlign:"center",marginTop:8}}>
                    <span style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:12}}>Wildcard subtotal: </span>
                    <span style={{color:C.accent,fontFamily:"'Bebas Neue',sans-serif",fontSize:16}}>{wcSubtotal} pts</span>
                  </div>
                </>
              );
            })()}
        ...
        {viewTab==="bracket"&&(
          <div style={{padding:12}}>
            ...
            {ROUNDS.map(round=>{
              ...
              const decidedCodes=new Set(results?.decided_teams?.[round.id]||[]);
              const anyDecided=decidedCodes.size>0;
              let roundPts=0;
              if(round)roundPts=Object.values(picks).filter(pick=>pick&&actualWinnerCodes.has(pick.code)).length*(round.pts||0);
              ...
              return(
                <div key={round.id} style={{marginBottom:14}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                    ...
                    {anyDecided&&<span style={{color:C.green,fontFamily:"'Bebas Neue',sans-serif",fontSize:14}}>+{roundPts} pts</span>}
                  </div>
                    ...
                    {Object.entries(picks).map(([idx,team])=>team?(()=>{
                      ...
                      return(
                        <div key={idx} style={{display:"flex",alignItems:"center",gap:6,background:C.card,borderRadius:7,padding:"7px 10px",border:"1px solid "+(isCorrect?C.green+"55":C.border),opacity:isEliminated?0.38:1}}>
                          ...
                          <FifaRank code={team.code}/>
                          {isCorrect&&<span style={{fontSize:10,color:C.green,flexShrink:0}}>{"+"+round.pts}</span>}
                          {isEliminated&&<span style={{fontSize:10,color:C.red,flexShrink:0}}>0</span>}
                          {isPending&&<span style={{fontSize:9,color:C.muted,flexShrink:0,fontStyle:"italic"}}>TBD</span>}
                        </div>
                      );
                    })():null)}
                  </div>
...
function BracketViewer({bracket,results,onClose}){
  ...
  const wc=results?.wildcard_codes||[];
  const GroupsTab=()=>(
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:8}}>
      {Object.keys(WC_GROUPS).map(g=>{
        const picks=gp[g]||WC_GROUPS[g],actual=gResults[g]||[];
        const details=picks.map((team,i)=>groupPickDetail(team,i,actual,wc,scoring));
        const exactCount=details.filter(d=>d.status==="exact").length;
        const perfect=actual.length>0&&exactCount===4;
        const gPts=details.reduce((acc,d)=>acc+(d.pts||0),0)+(perfect?scoring.perfectGroup:0);
        return(
          <div key={g} style={{background:C.card2,borderRadius:8,padding:10,border:"1px solid "+C.borderAccent}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
              ...
              {actual.length>0&&<span style={{color:C.accent,fontFamily:"'Bebas Neue',sans-serif",fontSize:13}}>+{gPts}</span>}
            </div>
            {picks.map((team,i)=>{
              const d=details[i];
              return(
            ...
            {perfect&&<div style={{color:C.green,fontFamily:"'Barlow',sans-serif",fontSize:10,marginTop:5,textAlign:"center"}}>{"+"+scoring.perfectGroup+" perfect bonus!"}</div>}
          </div>
        );
      })}
    </div>
    ...
    return(
      <div>
            ...
            <div key={round.id} style={{marginBottom:12}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:6}}>
                {Array.from({length:round.n},(_,i)=>{
                  const pick=picks[i];if(!pick)return null;
                  const status=knockoutPickStatus(pick.code,round.id,results);
                  const isCorrect=status==="correct",isElim=status==="eliminated",isPending=status==="pending";
                  if(isPending)pending++;else if(isCorrect)correct++;else eliminated++;
                  return(
                    <div key={i} style={{background:C.card2,borderRadius:7,padding:"7px 10px",display:"flex",alignItems:"center",gap:7,border:"0.5px solid "+C.border,opacity:isElim?0.55:1}}>
                      <Flag code={pick.code} size={18}/>
                      <span style={{color:isCorrect?C.accent:isElim?C.muted:C.text,fontFamily:"'Barlow',sans-serif",fontSize:11,flex:1,fontWeight:isCorrect?600:400,textDecoration:isElim?"line-through":"none"}}>{pick.name}</span>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginTop:10}}>
          {[[correct,C.green,"correct"],[eliminated,C.red,"eliminated"],[pending,C.muted,"pending"]].map(([v,c,l])=>(
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",zIndex:600,overflowY:"auto"}}>
      <div style={{maxWidth:900,margin:"0 auto",padding:12,paddingBottom:80}}>
        ...
        {tab==="groups"&&(
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:8}}>
            {Object.keys(WC_GROUPS).map(g=>{
              ...
              const myWc=results?.wildcard_codes||[];
              return(
                <div key={g} style={{background:C.card2,borderRadius:8,padding:10,border:"0.5px solid "+C.border}}>
                  ...
                  {[0,1,2,3].map(i=>{
                    ...
                    const myD=myT?groupPickDetail(myT,i,actual,myWc,scoring):{status:"pending"};
                    const opD=opT?groupPickDetail(opT,i,actual,myWc,scoring):{status:"pending"};
                    const colorFor=d=>d.status==="pending"?C.text:d.status==="none"?C.red:C.green;
                    ...
                    return(
                      <div key={i} style={{display:"flex",alignItems:"center",gap:5,padding:"4px 0",borderBottom:i<3?"0.5px solid "+C.border:"none"}}>
                        ...
                        <span style={{color:colorFor(myD),fontSize:10,flex:1}}>{myT?.name||"?"}</span>
                        {!same&&<span style={{color:C.amber,fontSize:9,flexShrink:0}}>{"!="}</span>}
                        {same&&<span style={{color:C.muted,fontSize:9,flexShrink:0}}>{"=="}</span>}
                        <span style={{color:colorFor(opD),fontSize:10,flex:1,textAlign:"right"}}>{opT?.name||"?"}</span>
        ...
        {tab==="knockout"&&(
          <div>
            {ROUNDS.map(round=>{
              const myPicks=myBracket.knockout_picks?.[round.id]||{},opPicks=opponent.knockout_picks?.[round.id]||{};
              ...
              return(
                <div key={round.id} style={{marginBottom:12}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:6}}>
                    {allIdx.map(idx=>{
                      const myPick=myPicks[+idx],opPick=opPicks[+idx],same=myPick?.code===opPick?.code;
                      const myStatus=myPick?knockoutPickStatus(myPick.code,round.id,results):"pending";
                      const opStatus=opPick?knockoutPickStatus(opPick.code,round.id,results):"pending";
                      const colorFor=s=>s==="correct"?C.green:s==="eliminated"?C.red:C.text;
                      return(
                        <div key={idx} style={{background:C.card2,borderRadius:7,padding:"8px 10px",border:"0.5px solid "+(same?C.border:C.amber+"44")}}>
                          <div style={{display:"flex",alignItems:"center",gap:6}}>
                            {myPick&&<><Flag code={myPick.code} size={16}/><span style={{color:colorFor(myStatus),fontSize:11,flex:1,textDecoration:myStatus==="eliminated"?"line-through":"none"}}>{myPick.name}</span></>}
                            {!same&&opPick&&<><span style={{color:C.amber,fontSize:10}}>{"!="}</span><span style={{color:colorFor(opStatus),fontSize:11,flex:1,textAlign:"right",textDecoration:opStatus==="eliminated"?"line-through":"none"}}>{opPick.name}</span><Flag code={opPick.code} size={16}/></>}
...
function InsightsPage({allBrackets,userId,results,picksVisible,matches}){
  ...
  const today=new Date().toDateString();
  const groupStageComplete=Object.keys(results?.group_results||{}).length>=12;
  const GroupNote=()=>!groupStageComplete?(
    <div style={{display:"flex",gap:10,background:"rgba(6,182,212,.06)",border:"1px solid rgba(6,182,212,.15)",borderRadius:10,padding:"12px 14px",marginBottom:14}}>
      <span style={{fontSize:16,flexShrink:0}}>{"⏳"}</span>
      <div>
        <div style={{color:C.accent,fontFamily:"'Bebas Neue',sans-serif",fontSize:12,letterSpacing:.5,marginBottom:3}}>GROUP STAGE IN PROGRESS</div>
        <div style={{color:C.muted,fontFamily:"'Barlow',sans-serif",fontSize:12,lineHeight:1.55}}>Score-based insights unlock around <strong style={{color:C.text}}>June 23</strong> once all 48 group matches finish. Pool picks and champion picks are available now.</div>
      </div>
    </div>
  ):null;
